 ...
 *
 *
 * bugfix:XHDataItem<*, out RecyclerView.ViewHolder>  都被改成了这样。否则会有类型转换问题
 */
class XHAdapter(context: Context) : Adapter<ViewHolder>() {
    private var recyclerViewRef: WeakReference<RecyclerView>? = null
    private var mContext: Context = context
    private var mInflater = LayoutInflater.from(context)
    private var dataSets = java.util.ArrayList<XHDataItem<*, out ViewHolder>>()

    //private var typeArrays = SparseArray<XHDataItem<*, out ViewHolder>>()
    ...
    fun addItemAt(
        index: Int,
        dataItem: XHDataItem<*, out ViewHolder>,
        notify: Boolean
    ) {
        if (index >= 0) {
     ...
     * 往现有集合的尾部逐年items集合
     */
    fun addItems(items: List<XHDataItem<*, out ViewHolder>>, notify: Boolean) {
     ...
     * 从指定位置上移除item
     */
    fun removeItemAt(index: Int): XHDataItem<*, out ViewHolder>? {
        if (index >= 0 && index < dataSets.size) {
            val remove: XHDataItem<*, out ViewHolder> = dataSets.removeAt(index)
     ...
     * 移除指定item
     */
    fun removeItem(dataItem: XHDataItem<*, out ViewHolder>) {
     ...
     * 指定刷新 某个item的数据
     */
    fun refreshItem(dataItem: XHDataItem<*, out ViewHolder>) {
...
//        }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        if (headers.indexOfKey(viewType) >= 0) {
            ...
            Log.d("RecyclerView", "Creating header ViewHolder for viewType: $viewType")
            return object : RecyclerView.ViewHolder(view) {}
        }
        if (footers.indexOfKey(viewType) >= 0) {
            val view = footers[viewType]
            Log.d("RecyclerView", "Creating footer ViewHolder for viewType: $viewType")
            return object : RecyclerView.ViewHolder(view) {}
        }
        ...
        Log.d(
            "RecyclerView",
            "Creating ViewHolder for dataItem: ${dataItem.javaClass} at position: $position"
        )
        val vh = dataItem.onCreateViewHolder(parent)
        if (vh != null) {
            Log.d("RecyclerView", "Using dataItem's ViewHolder: ${vh.javaClass}")
            return vh
        }
        ...
        Log.d(
            "RecyclerView",
            "Creating ViewHolder via createViewHolderInternal for dataItem: ${dataItem.javaClass}"
        )
    ...
    private fun createViewHolderInternal(
        javaClass: Class<XHDataItem<*, out ViewHolder>>,
        view: View
    ): ViewHolder {
        //得到该Item的父类类型,即为XHDataItem.class。  class 也是type的一个子类。
            ...
            for (argument in arguments) {
                if (argument is Class<*> && ViewHolder::class.java.isAssignableFrom(argument)) {
                    try {
                        ...
                        //这里需要  try-catch 一把，如果咱们直接在XHDataItem子类上标记 RecyclerView.ViewHolder，抽象类是不允许反射的
                        val viewHolder =
                            argument.getConstructor(View::class.java)
                                .newInstance(view) as ViewHolder
                        Log.d("RecyclerView", "Created ViewHolder: ${viewHolder.javaClass}")
                        return viewHolder
                    } catch (e: Throwable) {
                        Log.d("RecyclerView", "Created ViewHolder failed $e")
                        e.printStackTrace()
                    }
                }
            }
        }
        Log.d("RecyclerView", "Created default ViewHolder")
        return object : XHViewHolder(view) {}
    }
    ...
    fun getItem(position: Int): XHDataItem<*, ViewHolder>? {
        if (position < 0 || position >= dataSets.size)
            return null
        return dataSets[position] as XHDataItem<*, ViewHolder>
    }
